    ...
    steps:
      ...
      - name: Download PicoClaw binary
        run: |
          ...
          RELEASE_URL=$(gh api repos/${{ github.repository }}/releases/latest \
            --jq '.assets[] | select(.name | contains("linux-amd64")) | .browser_download_url')
      ...
      - name: Extract message
        ...
        run: |
          if [ "${{ github.event_name }}" = "issue_comment" ]; then
            ...
            BODY="${{ github.event.issue.title }}: ${{ github.event.issue.body }}"
          fi
      ...
      - name: Run PicoClaw agent
        ...
        run: |
          ...
          RESPONSE=$(./picoclaw agent \
            --message "${{ steps.message.outputs.body }}" \
            --session "$SESSION_KEY" 2>/dev/null)
...
This ensures concurrent agent runs append to rather than conflict on the memory file.
